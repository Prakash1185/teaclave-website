...
release checklist during the vote process by the Teaclave PMC. The `.asc` file
is a signature **created with the release manager's GPG key**. The `.sha512`
file contains a checksum for integrity verification.
...
The dist server uses SVN. Check it out locally, update the `KEYS` file, and
commit the change.
...
- Ensure the source code is ready for a new release, including bumping versions
...
- Review and follow the checklist in these cases:
  ...
  - If there are any changes to NOTICE, or LICENSE files, make sure they comply
...
### 2. Prepare for the Voting Email

In this stage, we prepare the contents which showed in the voting email:

- Preparing the three artifact files (`.tar.gz`, `.asc`, `.sha512`) and
  uploading them to dist/dev.
- Instructions for building and testing from source so others can validate the
  release source.

For example, the email template for TrustZone SDK is available
[here](https://github.com/apache/teaclave-trustzone-sdk/blob/main/docs/release-tips.md#email-template-for-voting).

Past voting email examples are provided in section 3.

#### 2.1 Prepare Artifacts: Sign, Verify, and Upload
...
Check out the directory, make the necessary updates, and upload. Be sure to
update the variables in the script to set the release version and use your ASF
username/password and GPG key ID.
...
# Check and set the variables in “Modify for Each Release”.
# For other projects, also edit the "Specific for Repo" section
vim make_release_artifacts.sh
...
#### 2.2 Instructions of Test Building from Source

For example, in the case of trustzone-sdk, please refer to the bash block in the
[mailing list email template](https://github.com/apache/teaclave-trustzone-sdk/blob/main/docs/release-tips.md#email-template-for-voting).

Before sending the voting email, please test the instructions to ensure they
work as expected.

For other projects, please adjust this based on the repository you're working
on.

### 3. Start Voting Process

Vote on the Apache Teaclave™ developers mailing list
...
- Each vote needs at least three binding +1 votes and more +1 votes than -1
  votes to pass.
- On `dev@teaclave.apache.org`, only votes from PMC members are binding.
- Votes must remain open for at least 72 hours (3 days). If there aren’t enough
  binding votes by then, extend the voting period.
- If a vote fails, update the release accordingly, create a new release
  candidate (with an increased rc number), and restart the voting process.
...
Once the vote passes, move the voted release files to the dist/release SVN
server.
...
- **In `dev`**: Ensure the final directory with artifacts exists at:
  <https://dist.apache.org/repos/dist/dev/teaclave/trustzone-sdk-0.4.0/> Also,
  delete all previous `0.4.0-rc` artifacts.
...
On the release edit page, use the tag `0.[X].0`, uncheck “Set as a pre-release”,
and submit.
...
- After tagging the release, new feature development is frozen for that release,
  but the `main` (development) branch remains active for ongoing work.
- If there are bug fixes for this release, cut the release branch from the tag,
  ...
  bug fixes from `main` as needed. All changes should land in `main` first, and
  then optionally be cherry-picked into the release branch. For example, if rc1
  requires changes based on voting feedback (e.g., `feedback-fix-1`): Suppose
  the `main` branch already has the following commits: `MAIN: feature (commit1)`
  ...
  - Then, cherry-pick `fix (commit2)` and `feedback-fix-1 (commit4)` into the
    release branch. The release branch becomes: `RELEASE: feature (commit1)` →
    `fix (commit2)` → `feedback-fix-1 (commit4)`.
- If a fix for the release isn’t compatible with `main`, it’s acceptable to
  apply the fix directly to the release branch. Please mark the commit message,
  e.g., "Fix specific to release-1.x; not applicable to main due to
